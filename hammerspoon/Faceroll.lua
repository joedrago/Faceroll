...
local function nextFrostMage()
    ...
    local winterschill = bitand(facerollGameBits, 0x1)
    local fingersoffrost = bitand(facerollGameBits, 0x2)
    local excessfire = bitand(facerollGameBits, 0x4)
    local excessfrost = bitand(facerollGameBits, 0x8)
    local glacialspike = bitand(facerollGameBits, 0x10)
    local frozenorb = bitand(facerollGameBits, 0x20)
    local cometstorm = bitand(facerollGameBits, 0x40)
    local flurry = bitand(facerollGameBits, 0x80)
    local shiftingpower = bitand(facerollGameBits, 0x100)
    ...
    if facerollAction == ACTION_Q then
        ...
        elseif glacialspike > 0 and cometstorm == 0 then
            --  Glacial Spike if Comet Storm is not ready
            sendKeyToWow("pad7") -- flurry
