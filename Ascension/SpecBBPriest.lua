-----------------------------------------------------------------------------------------
-- Ascension WoW Bronzebeard Priest

if Faceroll == nil then
    _, Faceroll = ...
end

local spec = Faceroll.createSpec("NUN", "cccccc", "PRIEST-ASCENSION")
...
local blastDeadzone = Faceroll.deadzoneCreate("Mind Blast", 1.5, 0.5)
...
spec.calcState = function(state)
    ...
    Faceroll.deadzoneUpdate(blastDeadzone)
    if Faceroll.deadzoneActive(blastDeadzone) then
        state.mindblast = false
    end
...
spec.calcAction = function(mode, state)
    if mode == Faceroll.MODE_ST then
        ...
        elseif state.targetingenemy then
            ...
            elseif state.darkfocus or (not state.coast and not state.combat and state.mindblast) then
                return "mindblast"
